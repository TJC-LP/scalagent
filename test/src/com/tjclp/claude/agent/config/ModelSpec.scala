...
class ModelSpec extends FunSuite:
  ...
  test("Model enum has correct IDs for Claude 4 family"):
  ...
  test("Model enum has correct IDs for Claude 3.5 family"):
    assertEquals(Model.Sonnet3_5.id, "claude-3-5-sonnet-latest")
    assertEquals(Model.Haiku3_5.id, "claude-3-5-haiku-latest")
  ...
  test("fromId parses known Claude 4 models"):
  ...
  test("fromId parses known Claude 3.5 models"):
    assertEquals(Model.fromId("claude-3-5-sonnet-latest"), Model.Sonnet3_5)
    assertEquals(Model.fromId("claude-3-5-haiku-latest"), Model.Haiku3_5)
  ...
  test("convenience aliases point to correct models"):
    assertEquals(Model.opus, Model.Opus4_5)
    assertEquals(Model.sonnet, Model.Sonnet4)
    assertEquals(Model.haiku, Model.Haiku3_5)

  test("default model is Sonnet4"):
    assertEquals(Model.default, Model.Sonnet4)
  ...
  test("Model encodes to JSON string"):
    assertEquals(Model.Sonnet4.toJson, """"claude-sonnet-4-20250514"""")
    assertEquals(Model.Opus4_5.toJson, """"claude-opus-4-5-20251101"""")

  test("Model decodes from JSON string"):
    assertEquals(""""claude-sonnet-4-20250514"""".fromJson[Model], Right(Model.Sonnet4))
