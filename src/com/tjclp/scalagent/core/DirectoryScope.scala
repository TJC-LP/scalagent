...
final case class DirectoryScope(
  ...
  def toHook: HookCallback =
    case input: HookInput.PreToolUse =>
      ...
      if toolName == ToolName.Bash then
        ZIO.succeed(HookOutput.denyTool(
  ...
  private[core] def isWithinScope(path: String): Boolean =
    ...
    val resolved = if nodePath.isAbsolute(path).asInstanceOf[Boolean] then
      nodePath.resolve(path).asInstanceOf[String]
    else
      nodePath.resolve(cwd, path).asInstanceOf[String]
...
object DirectoryScope:
  ...
  private[core] def extractScopeTargets(toolName: ToolName, toolInput: Json, cwd: String): List[String] =
  ...
  private def resolveGlobTarget(cwd: String, searchPath: Option[String], pattern: String): String =
    searchPath match
      case Some(path) =>
        val base =
