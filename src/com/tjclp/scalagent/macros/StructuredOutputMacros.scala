...
object StructuredOutputMacros:
  ...
  private def generateSchemaExpr(
    ...
    val propertiesExprs: List[Expr[(String, Json)]] = fieldInfos.map {
  ...
  private def typeToSchemaExpr(using Quotes)(tpe: quotes.reflect.TypeRepr): Expr[Json] =
    ...
    tpe.dealias match
      ...
      case AppliedType(tycon, List(elemType)) if isListLike(tycon) =>
        ...
        if isSetLike(tycon) then '{ JsonSchemaAst.array(items = $elemSchemaExpr, uniqueItems = true) }
        else '{ JsonSchemaAst.array($elemSchemaExpr) }
