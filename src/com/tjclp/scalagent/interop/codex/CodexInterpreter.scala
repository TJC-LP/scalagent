...
import com.tjclp.scalagent.codex.{CodexClient, CodexEvent, CodexInput, CodexThreadOptions, CodexTurnOptions, SandboxMode}
...
object CodexInterpreter:
  ...
  def typed[A](
      client: CodexClient,
      threadOptions: CodexThreadOptions = CodexThreadOptions.default
  )(using StructuredOutput[A]): Agent[Any, CodexInput, A] =
  ...
  def typedBuilder[A](
      client: CodexClient,
      threadOptions: CodexThreadOptions = CodexThreadOptions.default
  )(using so: StructuredOutput[A]): AgentBuilder[Any, CodexInput, A, Any] =
    AgentBuilder.withTransform(
  ...
  )(using codec: OutputCodec[O]): (Agent[Any, CodexInput, O], BuilderConfig) => Agent[Any, CodexInput, O] =
    (_, cfg) =>
      require(
  ...
  )(using codec: OutputCodec[O]): Agent[Any, CodexInput, O] =
    new Agent[Any, CodexInput, O]:
      def run(principal: Any, input: CodexInput, policy: ExecutionPolicy): AgentRun[Any, O] =
          ...
          Promise.make[Nothing, SharedRun[O]].flatMap { myPromise =>
            stateRef.modify {
              ...
              case SharedState.Empty() =>
                val init = buildSharedRun(client, input, effectiveOptions, turnOptions, policy, codec).flatMap { shared =>
  ...
  private def buildSharedRun[O](
    ...
    for
      ...
      eventStream = thread.runStreamed(input, turnOptions)
