...
object ClaudeInterpreter:
  ...
  ): (Agent[Any, String, O], BuilderConfig) => Agent[Any, String, O] =
    (_, cfg) =>
      ...
      val opts =
        if toolSurface.isEmpty && mcpToolSurfaces.isEmpty then baseOptions.withSafeToolDefault
          ...
          allMcpToolSurfaces.foldLeft(
            baseOptions.copy(
              allowedTools = Some(toolSurface.distinctAllowedTools)
            )
          ) { (o, surface) => o.withMcpServer(surface.serverName, McpToolLoader.toServerFactory(surface, runtime)) }
  ...
  private def buildSharedRun[O](
    ...
    for
      ...
      sink           = (message: AgentMessage) =>
        ZIO.foreachDiscard(EventMapper.mapMessage(message)) { event => queue.offer(Take.single(event)).unit }
      query = applyDeadline(
  ...
  private def overlayPolicy[O](
    ...
    // Max turns
    policy.maxTurns.foreach { turns => opts = opts.copy(maxTurns = Some(turns)) }
    ...
    codec.structuredOutputFormat.foreach { so => opts = opts.copy(outputFormat = Some(OutputFormat(so.jsonSchema))) }
