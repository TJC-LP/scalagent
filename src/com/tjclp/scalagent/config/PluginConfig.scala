  ...
  def localValidated(path: String): IO[PluginError, PluginConfig] =
    ZIO
      .attempt {
        ...
        // Check path exists
        if !NodeFs.existsSync(path) then throw PathNotFound(path)

        // Check is directory
        val stat = NodeFs.statSync(path)
        if !stat.isDirectory().asInstanceOf[Boolean] then throw NotADirectory(path)

        // Check manifest exists
        val manifestPath = s"$path/.claude-plugin/plugin.json"
        if !NodeFs.existsSync(manifestPath) then throw MissingManifest(path, manifestPath)
