...
object A2ATool:
  ...
  def fromClient(
    ...
    ToolDef.fromInput[Input](
      ...
      client
        .sendAndPollText(input.message, pollEvery = 5.seconds)
        .map { task =>
          val response = task.status.message.map(_.text).getOrElse("No response")
          ToolResult.text(response)
        }
        .catchAll { error => ZIO.succeed(ToolResult.error(s"A2A call failed: ${error.getMessage}")) }
    }
  ...
  def createServerFromClients(
    ...
    val tools = clients.map {
      case (name, description, client) =>
