...
object Utility:
  ...
  def reliability[P, O]: Utility[P, O] =
    (_,
      _,
      trace,
    ) => if trace.isSuccess then 1.0 else 0.0
  ...
  def latencyMinimizing[P, O]: Utility[P, O] =
    (_,
      _,
      trace,
    ) => 1.0 / (1.0 + trace.durationMs.toDouble / 1000.0)
