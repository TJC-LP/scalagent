...
private[a2a] final class A2AServerLiveImpl(
  ...
  private val activeRuns = TrieMap.empty[TaskRuntimeKey, Fiber.Runtime[Throwable, Unit]]
  ...
  private[a2a] def dispatchJsonRpc(request: JsonRpcRequest): Task[JsonRpcResponse] =
    request.method match
      ...
      case A2AMethod.TasksCancel =>
        paramsAs[A2ARequest.TasksCancel](request).flatMap(handleCancelTask).map(JsonRpcResponse.success(request.id, _))
  ...
  private def handleSendMessage(req: A2ARequest.MessageSend): Task[A2AResponse.SendMessageResult] =
    ...
    val tenant  = req.tenant
    val key     = taskRuntimeKey(taskId, ServerCallContext(tenant = tenant))
    config.executionOverride match
      case Some(executor) =>
        ...
        for
          ...
          run = executor(message, taskId, ctxId, publisher).catchAll(e =>
            publisher.publish(
          ...
          fiber <- run.fork
          _ = activeRuns.put(key, fiber)
          _ <- fiber.await
          _ = activeRuns.remove(key, fiber)
          _ <- publisher.finish
          events = publisher.snapshot
          current <- taskStoreEff.load(taskId, tenant)
          finalTask = current match
            case Some(task) if task.status.state == TaskState.Canceled => task
            case _                                                     =>
              events.foldLeft(initial) { (t, ev) =>
                ev match
          ...
          _ <- ZIO.unless(finalTask.status.state == TaskState.Canceled)(taskStoreEff.save(finalTask, tenant))
  ...
  private def handleCancelTask(req: A2ARequest.TasksCancel): Task[A2ATask] =
    val tenant = req.tenant
    val key    = taskRuntimeKey(req.id, ServerCallContext(tenant = tenant))
    taskStoreEff.load(req.id, tenant).flatMap {
      case Some(task) if task.isTerminal =>
        ZIO.fail(A2AError.taskNotCancelable(req.id))
      case Some(task) =>
        val canceled = task.copy(status = TaskStatus.canceled)
        for
          _ <- taskStoreEff.save(canceled, tenant)
          maybeFiber = activeRuns.remove(key)
          _ <- ZIO.foreachDiscard(maybeFiber)(_.interrupt)
        yield canceled
      case None =>
        ZIO.fail(A2AError.taskNotFound(req.id))
    }
