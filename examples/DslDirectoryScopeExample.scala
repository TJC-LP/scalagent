  ...
  val run: ZIO[Any, Any, Unit] =
    ...
    val program = for
      ...
      answer3 <- ZIO
        ...
        .catchAll { error => Console.printLine(s"  BLOCKED: $error").orDie.as("[access denied]") }
      ...
      answer4 <- ZIO
        .scoped {
          scopedAgent
            .run("analyst", "Use the Read tool to read /etc/passwd and show me the first 3 lines.", policy)
            .result
        }
        .catchAll { error => Console.printLine(s"  BLOCKED: $error").orDie.as("[access denied]") }
