...
object SlashCommandExample extends ZIOAppDefault:
  ...
  val run: ZIO[Any, Any, Unit] =
    for
      ...
      _ <- Console
        .printLine(
          s"   compact: value='${compactCmd.value}', withSlash='${compactCmd.withSlash}', isBuiltin=${compactCmd.isBuiltin}"
        )
        .orDie
      _ <- Console
        .printLine(
          s"   clear:   value='${clearCmd.value}', withSlash='${clearCmd.withSlash}', isBuiltin=${clearCmd.isBuiltin}"
        )
        .orDie
      _ <- Console
        .printLine(
          s"   help:    value='${helpCmd.value}', withSlash='${helpCmd.withSlash}', isBuiltin=${helpCmd.isBuiltin}\n"
        )
        .orDie
      ...
      _ <- Console
        .printLine(s"   deploy:    withSlash='${deployCmd.withSlash}', isBuiltin=${deployCmd.isBuiltin}")
        .orDie
      _ <- Console
        .printLine(s"   test-all:  withSlash='${testAllCmd.withSlash}', isBuiltin=${testAllCmd.isBuiltin}")
        .orDie
      _ <- Console
        .printLine(s"   review-pr: withSlash='${reviewPrCmd.withSlash}', isBuiltin=${reviewPrCmd.isBuiltin}\n")
        .orDie
      ...
      _ <- Console
        .printLine(
          "       case AgentMessage.System(SystemEvent.Init(_, _, _, _, _, _, _, slashCommands, _, _, _, _, _), _, _) =>"
        )
        .orDie
